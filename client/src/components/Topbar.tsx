    ...
    return (
        <main className="text-white bg-black py-6">
            <div className="max-w-6xl mx-auto flex items-center justify-between">
                <Link to={"/"}><h2 className="font-extrabold text-3xl">FASH.COM</h2></Link>
                <SearchBox />
                <div className="flex items-center gap-4 cursor-pointer">
                    <ShoppingCart onClick={toggleCart} />
                    {
                        userInfo ? <DropdownMenu>
                            <DropdownMenuTrigger className="cursor-pointer"><User /></DropdownMenuTrigger>
                            <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem className="cursor-pointer" onClick={logoutHandler} disabled={isLoading}>
                                    <span className="text-red-600">Logout</span>
                                </DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu> : <Link to={"/login"} ><LogIn /></Link>
                    }
                </div>
            </div>
        </main >
